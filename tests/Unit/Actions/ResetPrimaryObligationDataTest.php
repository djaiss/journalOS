...
use App\Actions\ResetPrimaryObligationData;
...
final class ResetPrimaryObligationDataTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function it_resets_primary_obligation_data_for_a_journal_entry(): void
    {
        ...
        $modulePrimaryObligation = ModulePrimaryObligation::factory()->create([
            'journal_entry_id' => $entry->id,
            'primary_obligation' => 'work',
        ]);

        $result = new ResetPrimaryObligationData(
            user: $user,
            entry: $entry,
        )->execute();

        $this->assertNull($result->modulePrimaryObligation?->primary_obligation);
        $this->assertDatabaseMissing('module_primary_obligation', [
            'id' => $modulePrimaryObligation->id,
        ]);
        ...
        Queue::assertPushedOn(
            ...
            callback: fn (LogUserAction $job) => $job->action === 'primary_obligation_reset' && $job->user->id === $user->id,
        );
    ...
    public function it_throws_when_entry_does_not_belong_to_user(): void
    {
        ...
        new ResetPrimaryObligationData(
            user: $user,
            entry: $entry,
        )->execute();
    }
