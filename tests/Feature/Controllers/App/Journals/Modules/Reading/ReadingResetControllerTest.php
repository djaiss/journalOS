...
final class ReadingResetControllerTest extends TestCase
{
    use RefreshDatabase;

    #[Test]
    public function it_resets_reading_data_and_redirects(): void
    {
        ...
        $response = $this->actingAs($user)->put(
            "/journals/{$journal->slug}/entries/2022/1/1/reading/reset",
        );

        $response->assertRedirectContains("/journals/{$journal->slug}/entries/2022/1/1");
        $response->assertSessionHas('status');

        $entry->refresh()->load('moduleReading', 'books');
        $this->assertNull($entry->moduleReading);
    ...
    public function it_redirects_guests_to_login(): void
    {
        ...
        $response = $this->put("/journals/{$journal->slug}/entries/2022/1/1/reading/reset");
