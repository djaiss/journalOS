...
final class JournalModulesSettingsControllerTest extends TestCase
{
    use RefreshDatabase;

    #[Test]
    public function it_shows_the_journal_modules_settings_page(): void
    {
        ...
        $response = $this->actingAs($user)->get('/journals/' . $journal->slug . '/settings/modules');
    ...
    public function it_returns_not_found_if_user_does_not_own_the_journal(): void
    {
        ...
        $response = $this->actingAs($user)->get('/journals/' . $otherJournal->slug . '/settings/modules');
