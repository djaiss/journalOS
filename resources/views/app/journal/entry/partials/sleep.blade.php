        ...
        @foreach ($module['bedtime'] as $option)
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="sleep-container notifications reset" :action="$module['bedtime_update_url']" method="put" class="h-full">
              ...
              <button type="submit" class="{{ $option['is_selected'] ? 'bg-green-50 font-bold dark:bg-green-900/40' : '' }} first:rounded-l-lg last:rounded-r-lg flex h-full w-full cursor-pointer items-center justify-center p-2 text-center hover:bg-green-50 dark:hover:bg-green-900/40">
        ...
        @foreach ($module['wake_up_time'] as $option)
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="sleep-container notifications reset" :action="$module['wake_up_time_update_url']" method="put" class="h-full">
              ...
              <button type="submit" class="{{ $option['is_selected'] ? 'bg-green-50 font-bold dark:bg-green-900/40' : '' }} flex first:rounded-l-lg last:rounded-r-lg h-full w-full cursor-pointer items-center justify-center p-2 text-center hover:bg-green-50 dark:hover:bg-green-900/40">
                {{ \App\Helpers\TimeHelper::format($option['time']) }}
              </button>
            </x-form>
          </div>
