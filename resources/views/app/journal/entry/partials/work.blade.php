...
<x-module>
  ...
  <div id="work-container" class="space-y-4" x-data="{
    showWorkDetails: {{ $module['worked'] === 'yes' ? 'true' : 'false' }},
  }">
    <div>
      ...
      <div class="space-y-2">
        ...
        <div class="flex w-full rounded-lg border border-gray-200 dark:border-gray-700">
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="work-container notifications work-reset days-listing months-listing" :action="$module['work_url']" method="put" class="h-full">
              <input type="hidden" name="worked" value="yes" />
              <button @click="showWorkDetails = true" type="submit" class="{{ $module['worked'] === 'yes' ? 'bg-green-50 font-bold dark:bg-green-900/40' : '' }} flex h-full w-full cursor-pointer items-center justify-center p-2 text-center first:rounded-l-lg last:rounded-r-lg hover:bg-green-50 dark:hover:bg-green-900/40">
                {{ __('Yes') }}
              </button>
            </x-form>
          </div>
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="work-container notifications work-reset days-listing months-listing" :action="$module['work_url']" method="put" class="h-full">
              ...
              <button @click="showWorkDetails = false" type="submit" class="{{ $module['worked'] === 'no' ? 'bg-red-50 font-bold dark:bg-red-900/40' : '' }} flex h-full w-full cursor-pointer items-center justify-center p-2 text-center first:rounded-l-lg last:rounded-r-lg hover:bg-red-50 dark:hover:bg-red-900/40">
                {{ __('No') }}
              </button>
            </x-form>
          </div>
      ...
      <div x-show="showWorkDetails" x-cloak class="mt-4 space-y-4">
        ...
        <div class="space-y-2">
          ...
          <div class="flex w-full rounded-lg border border-gray-200 dark:border-gray-700">
            <x-button.yes name="work_procrastinated" value="yes" x-target="work-container notifications work-reset days-listing months-listing" :action="$module['work_url']" selected="{{ $module['work_procrastinated'] === 'yes' }}" />
            <x-button.no name="work_procrastinated" value="no" x-target="work-container notifications work-reset days-listing months-listing" :action="$module['work_url']" selected="{{ $module['work_procrastinated'] === 'no' }}" />
          </div>
        </div>
      </div>
    </div>
