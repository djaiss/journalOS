  ...
  <x-slot:action>
    <div id="mood-reset">
      @if ($module['display_reset'])
        <x-form x-target="mood-container notifications mood-reset days-listing months-listing" :action="$module['reset_url']" method="put">
      ...
      <div class="flex w-full rounded-lg border border-gray-200 dark:border-gray-700">
        @foreach ($module['mood_options'] as $option)
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="mood-container notifications mood-reset days-listing months-listing" :action="$module['mood_url']" method="put">
              <input type="hidden" name="mood" value="{{ $option['value'] }}" />
