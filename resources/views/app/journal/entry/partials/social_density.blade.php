...
<x-module>
  ...
  <x-slot:emoji>👥</x-slot>
  <x-slot:action>
    <div id="social-density-reset">
      @if ($module['display_reset'])
        <x-form x-target="social-density-container notifications social-density-reset days-listing months-listing" :action="$module['reset_url']" method="put">
  ...
  <div id="social-density-container">
    <div class="space-y-2">
      <p>{{ __('How was your social density today?') }}</p>
      <div class="flex w-full rounded-lg border border-gray-200 dark:border-gray-700">
        @foreach ($module['social_density_options'] as $option)
          <div class="flex-1 border-r border-gray-200 first:overflow-hidden first:rounded-l-lg last:rounded-r-lg last:border-r-0 dark:border-gray-700">
            <x-form x-target="social-density-container notifications social-density-reset days-listing months-listing" :action="$module['social_density_url']" method="put">
              <input type="hidden" name="social_density" value="{{ $option['value'] }}" />
