    ...
    <div
      ...
      x-data="{
        ...
        reorder(columnNumber, position, sameColumnPosition = null) {
          ...
          const moduleIndex = this.columns[sourceColumn].findIndex(
            (m) => m.key === this.dragging,
          )
        ...
        removeModule(moduleKey, columnNumber) {
          const index = this.columns[columnNumber].findIndex(
            (m) => m.key === moduleKey,
          )
          if (index !== -1) {
          ...
          <div class="flex flex-1 flex-col gap-4">
            <div class="flex min-h-[16rem] flex-col rounded-lg border border-gray-200 bg-white p-4 shadow-xs dark:border-gray-800 dark:bg-gray-900">
              ...
              <div class="flex flex-col gap-2">
                <template x-for="(module, index) in columns[colNum]" :key="module.key">
                  <div draggable="true" @dragstart="startDrag(module.key, colNum)" @dragend="clearDrag()" @dragover.prevent="markDropTarget(colNum, index + 1)" @drop.prevent="reorder(colNum, index + 1)" :class="dropTarget === `${colNum}:${index + 1}` ? 'border-blue-400 ring-2 ring-blue-200 dark:ring-blue-900/40' : 'border-gray-200 dark:border-gray-700'" class="group flex items-center justify-between rounded-lg border bg-white px-3 py-2 text-sm text-gray-700 shadow-xs transition dark:bg-gray-950 dark:text-gray-100">
                    ...
                    <button type="button" @click="removeModule(module.key, colNum)" class="relative inline-flex h-8 transform-gpu cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-300 border-b-gray-300/80 bg-white text-sm text-xs font-medium whitespace-nowrap text-gray-800 shadow-xs transition duration-150 ease-out hover:border-gray-400 hover:bg-gray-50 active:translate-y-[1px] active:scale-[0.97] active:shadow-inner active:ease-in disabled:pointer-events-none disabled:cursor-default disabled:opacity-75 aria-pressed:z-10 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:hover:border-gray-600 dark:hover:bg-gray-600/75 dark:disabled:opacity-75 [:where(&)]:px-3">Remove</button>
                  </div>
                </template>

                <div class="rounded-lg border border-dashed border-gray-200 bg-gray-50 p-2 text-center text-xs text-gray-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400" @dragover.prevent="markDropTarget(colNum, getColumnCount(colNum) + 1)" @drop.prevent="reorder(colNum, getColumnCount(colNum) + 1, getColumnCount(colNum))" :class="dropTarget === `${colNum}:${getColumnCount(colNum) + 1}` ? 'border-blue-400 text-blue-700 dark:text-blue-200' : ''">Drop here to move</div>
              </div>
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-xs dark:border-gray-800 dark:bg-gray-900">
              <div class="text-center" x-show="!showAddForm[colNum]">
                <button type="button" @click="showAddForm[colNum] = true" class="relative inline-flex h-8 transform-gpu cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-300 border-b-gray-300/80 bg-white text-sm font-medium whitespace-nowrap text-gray-800 shadow-xs transition duration-150 ease-out hover:border-gray-400 hover:bg-gray-50 active:translate-y-[1px] active:scale-[0.97] active:shadow-inner active:ease-in disabled:pointer-events-none disabled:cursor-default disabled:opacity-75 aria-pressed:z-10 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:hover:border-gray-600 dark:hover:bg-gray-600/75 dark:disabled:opacity-75 [:where(&)]:px-3">
                  <span class="shrink-0">
              ...
              <div x-show="showAddForm[colNum]" style="display: none">
                <div class="flex flex-col gap-2">
                  <div class="space-y-2">
                    ...
                    <select :id="`layout-module-${colNum}`" x-model="selectedModule[colNum]" class="block h-10 w-full appearance-none rounded-lg border border-gray-200 border-b-gray-300/80 bg-white px-3 py-2 text-base leading-[1.375rem] text-gray-700 shadow-xs aria-invalid:border-red-500 sm:text-sm dark:border-white/10 dark:bg-white/10 dark:text-gray-300 dark:shadow-none" required>
                  ...
                  <div class="mt-2 flex items-center justify-between gap-2">
                    <button type="button" @click="showAddForm[colNum] = false; selectedModule[colNum] = ''" class="relative inline-flex h-8 transform-gpu cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-300 border-b-gray-300/80 bg-white text-sm font-medium whitespace-nowrap text-gray-800 shadow-xs transition duration-150 ease-out hover:border-gray-400 hover:bg-gray-50 active:translate-y-[1px] active:scale-[0.97] active:shadow-inner active:ease-in disabled:pointer-events-none disabled:cursor-default disabled:opacity-75 aria-pressed:z-10 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:hover:border-gray-600 dark:hover:bg-gray-600/75 dark:disabled:opacity-75 [:where(&)]:px-3">Cancel</button>
                    <button type="button" @click="addModule(colNum)" class="relative inline-flex h-8 cursor-pointer items-center justify-center gap-2 rounded-lg border border-black/10 bg-[var(--color-accent)] text-sm font-medium whitespace-nowrap text-[var(--color-accent-foreground)] shadow-[inset_0px_1px_--theme(--color-white/.2)] transition-[transform,box-shadow] duration-150 hover:bg-[color-mix(in_oklab,_var(--color-accent),_transparent_10%)] focus-visible:ring-2 focus-visible:ring-[var(--color-accent)]/50 focus-visible:outline-none active:scale-[0.97] active:shadow-[inset_0_2px_4px_0_rgba(0,0,0,0.35),inset_0_0_0_1px_rgba(0,0,0,0.25)] disabled:pointer-events-none disabled:cursor-default disabled:opacity-75 aria-pressed:z-10 dark:border-0 dark:disabled:opacity-75 [:where(&)]:px-3">
                      Add
                    </button>
                  </div>
                </div>
