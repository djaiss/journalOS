    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="mb-6 overflow-hidden rounded-lg border border-gray-200 bg-white">
        <div class="flex items-center justify-between border-b border-gray-200 p-3">
          ...
          <span class="font-mono text-lg text-gray-900">{{ $user->id }}</span>
        </div>
        <div class="grid grid-cols-4 divide-x divide-gray-200">
          ...
          <div class="p-6">
            <div class="flex flex-col gap-2">
              ...
              <span class="font-mono text-gray-900">{{ $user->created_at->format('F d, Y') }}</span>
            </div>
          </div>
          ...
          <div class="p-6">
            <div class="flex flex-col gap-2">
              ...
              <span class="font-mono text-gray-900">{{ $user->last_activity_at->format('F d, Y') }}</span>
            </div>
          </div>
            ...
            <div class="flex flex-col gap-3">
              ...
              @else
                <div class="space-y-1.5">
                  ...
                  <p class="text-sm text-gray-600">{{ $user->trial_ends_at->diffInDays() }} {{ __('days remaining') }}</p>
                </div>
              @endif
            </div>
          </div>
      ...
      {{--
        ...
        <!-- Delete Account -->
        <form onsubmit="return confirm('Are you absolutely sure? This action cannot be undone.')" action="{{ route('instance.destroy', $user) }}" method="post" class="w-full">
        @csrf
        @method('delete')
        ...
        <div class="divide-y divide-gray-200">
          @foreach ($logs as $log)
            <div class="flex items-center justify-between p-4">
              <div class="flex items-center gap-3">
                <x-phosphor-pulse class="h-4 w-4 text-gray-500" />
                <div>
