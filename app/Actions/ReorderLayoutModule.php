<?php

declare(strict_types = 1);

namespace App\Actions;

use App\Helpers\TextSanitizer;
use App\Jobs\LogUserAction;
use App\Jobs\UpdateUserLastActivityDate;
use App\Models\Layout;
use App\Models\LayoutModule;
use App\Models\User;
use Illuminate\Database\Eloquent\ModelNotFoundException;
use Illuminate\Support\Facades\DB;
use Illuminate\Validation\ValidationException;

final class ReorderLayoutModule
{
    private LayoutModule $layoutModule;

    public function __construct(
        private readonly User $user,
        private readonly Layout $layout,
        private string $moduleKey,
        private readonly int $columnNumber,
        private readonly int $position,
    ) {}

    public function execute(): LayoutModule
    {
        $this->validate();
        $this->reorder();
        $this->updateUserLastActivityDate();
        $this->log();

        return $this->layoutModule->refresh();
    }

    private function validate(): void
    {
        $this->moduleKey = mb_strtolower(TextSanitizer::plainText($this->moduleKey));

        if ($this->layout->journal->user_id !== $this->user->id) {
            throw new ModelNotFoundException('Layout not found');
        }

        if ($this->moduleKey === '' || !in_array($this->moduleKey, LayoutModule::allowedModuleKeys(), true)) {
            throw ValidationException::withMessages([
                'module_key' => 'Module key is invalid',
            ]);
        }

        if ($this->columnNumber < 1 || $this->columnNumber > $this->layout->columns_count) {
            throw ValidationException::withMessages([
                'column_number' => 'Column number must be within the layout columns',
            ]);
        }

        $this->layoutModule = LayoutModule::query()
            ->where('layout_id', $this->layout->id)
            ->where('module_key', $this->moduleKey)
            ->first() ?? throw ValidationException::withMessages([
            'module_key' => 'Module does not exist in layout',
        ]);

        $columnCount = LayoutModule::query()
            ->where('layout_id', $this->layout->id)
            ->where('column_number', $this->columnNumber)
            ->count();

        $maxPosition = $this->layoutModule->column_number === $this->columnNumber
            ? $columnCount
            : $columnCount + 1;

        if ($this->position < 1 || $this->position > $maxPosition) {
            throw ValidationException::withMessages([
                'position' => 'Position is invalid for the selected column',
            ]);
        }
    }

    private function reorder(): void
    {
        $currentColumn = $this->layoutModule->column_number;
        $currentPosition = $this->layoutModule->position;

        DB::transaction(function () use ($currentColumn, $currentPosition): void {
            if ($currentColumn === $this->columnNumber && $currentPosition === $this->position) {
                return;
            }

            if ($currentColumn === $this->columnNumber) {
                $this->reorderWithinColumn($currentColumn, $currentPosition);
                $this->layoutModule->update([
                    'position' => $this->position,
                ]);

                return;
            }

            LayoutModule::query()
                ->where('layout_id', $this->layout->id)
                ->where('column_number', $currentColumn)
                ->where('position', '>', $currentPosition)
                ->decrement('position');

            LayoutModule::query()
                ->where('layout_id', $this->layout->id)
                ->where('column_number', $this->columnNumber)
                ->where('position', '>=', $this->position)
                ->increment('position');

            $this->layoutModule->update([
                'column_number' => $this->columnNumber,
                'position' => $this->position,
            ]);
        });
    }

    private function reorderWithinColumn(int $columnNumber, int $currentPosition): void
    {
        if ($this->position > $currentPosition) {
            LayoutModule::query()
                ->where('layout_id', $this->layout->id)
                ->where('column_number', $columnNumber)
                ->whereBetween('position', [$currentPosition + 1, $this->position])
                ->decrement('position');

            return;
        }

        LayoutModule::query()
            ->where('layout_id', $this->layout->id)
            ->where('column_number', $columnNumber)
            ->whereBetween('position', [$this->position, $currentPosition - 1])
            ->increment('position');
    }

    private function log(): void
    {
        $journal = $this->layout->journal;

        LogUserAction::dispatch(
            user: $this->user,
            journal: $journal,
            action: 'layout_module_reorder',
            description: sprintf(
                'Reordered the %s module in the layout %s for the journal %s',
                $this->moduleKey,
                $this->layout->name,
                $journal->name,
            ),
        )->onQueue('low');
    }

    private function updateUserLastActivityDate(): void
    {
        UpdateUserLastActivityDate::dispatch($this->user)->onQueue('low');
    }
}
